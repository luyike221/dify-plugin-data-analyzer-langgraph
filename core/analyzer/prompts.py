...
3. **分析任务数量限制**：
   - **最多3个分析任务**：`analysis_tasks` 数组中的任务数量**不得超过3个**
   - 如果需求复杂，优先选择**最重要、最核心的3个分析任务**
   - 将相关或相似的分析任务合并为一个任务，避免任务过多
   - 确保每个任务都是独立且可执行的
4. **禁止自动进行基本结构分析**：
   - **不要**自动包含数据基本结构分析任务（如：行数统计、列数统计、各列数据类型分析、缺失值统计等）
   - **除非用户明确要求**（如"有多少行数据？"、"检查缺失值"等），否则不要在分析计划中包含这些基本结构分析
   - 直接针对用户的具体需求进行分析，跳过基本结构分析步骤
5. **澄清规则**：
...
1. **一次性完成多个分析任务**：
   - 如果提供了多个分析任务，**必须一次性生成包含所有任务的代码**
   - 不要分多次执行，不要只完成第一个任务
   - 每个分析任务的结果用清晰的标题分隔（如：`print("\\n=== 任务1：XXX ===\\n")`）
   - 确保所有分析任务都在同一段代码中完成
2. **禁止硬编码结果**：所有 print 必须输出计算结果，不能输出预设结论
3. **禁止自动进行基本结构分析**：
   - **不要**自动输出数据的基本结构信息（如：行数、列数、各列数据类型、缺失值统计等）
   - **除非分析任务明确要求**，否则不要在代码中包含 `df.shape`、`df.info()`、`df.isnull().sum()` 等基本结构分析代码
   - 直接执行分析任务，跳过基本结构分析步骤
4. **注意单位**：
   - 列名含 `(%)` 的，计算时需除以100
   - 列名含 `(万元)` 的，转换为元需乘以10000
5. **禁止生成图片**：不使用 plt.show()
...
请编写 Python 代码完成以上所有分析任务。注意：
1. 一次性完成所有分析任务，不要分多次执行
2. 所有结果必须通过 print() 输出，禁止硬编码结论
3. 每个分析任务的结果用清晰的标题分隔，便于区分
"""
...
"""
    ...
    REPORT_GENERATION_SYSTEM = """你是专业的数据分析报告撰写专家。根据用户的问题和分析结果，生成详细、深入的分析报告。

## 核心原则

1. **严格基于数据**：
   - **所有结论、数字、统计结果必须来自提供的分析结果**
   - **绝对禁止编造、推测或假设任何数据**
   - 如果分析结果中没有某个信息，明确说明"数据中未提供此信息"
   - 所有引用的数据都要有明确的来源（来自哪轮分析结果）

2. **详细深入**：
   - 对分析结果进行深入解读和说明
   - 提供多角度的分析和洞察
   - 解释数据背后的含义和趋势
   - 进行合理的对比和关联分析（但必须基于实际数据）
...
- 所有数字必须来自分析结果，格式：**具体数值**（来自第X轮分析）
- 如果某个维度没有数据，明确说明："该维度在分析结果中未提供数据"
- 不要使用模糊表述（如"很多"、"较少"），要使用具体数字
...
**请根据用户问题和上述分析结果，生成一份详细、深入的分析报告。**

**重要要求：**
1. **严格基于数据**：所有结论、数字、统计结果必须来自上述分析结果，绝对禁止编造任何数据
2. **详细深入**：对分析结果进行深入解读，提供多角度的分析和洞察
3. **全面覆盖**：综合所有分析结果，不要遗漏重要发现
4. **结构清晰**：使用清晰的章节结构，每个发现都要有数据支撑
5. **引用来源**：引用数据时注明来源（如"来自第1轮分析"）

**报告要求：**
- 对于简单查询：直接给出答案，并提供相关背景信息
- 对于分析类问题：生成包含核心发现、详细分析、数据洞察、关键指标、总结等部分的完整报告
- 使用表格、列表等格式增强可读性
- 所有数字必须来自分析结果，格式：**具体数值**（来自第X轮分析）
- 如果某个信息在分析结果中没有，明确说明"数据中未提供此信息"

**请生成一份详细、专业、严格基于数据的分析报告。**
"""
...
"""
    ...
    def format_code_generation_prompt(
        ...
        Args:
            ...
            analysis_tasks: 分析任务列表（首轮时使用，包含多个分析任务）
        """
        data_info = cls._format_data_info(
            csv_path, row_count, column_names, column_metadata, data_preview
        )
        ...
        if is_first_round or not previous_results:
            # 首轮：如果有多个分析任务，一次性生成包含所有任务的代码
            if analysis_tasks and len(analysis_tasks) > 1:
                # 格式化多个分析任务
                tasks_str = "\n".join([f"{i+1}. {task}" for i, task in enumerate(analysis_tasks)])
                ...
                # 单个任务或没有任务列表，使用原来的逻辑
                task = analysis_tasks[0] if analysis_tasks and len(analysis_tasks) > 0 else user_prompt
                tasks_str = f"1. {task}"
                user_content = cls.CODE_GENERATION_USER_FIRST.format(
                    data_info=data_info,
                    analysis_tasks=tasks_str,
