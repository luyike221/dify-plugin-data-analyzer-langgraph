...
class Storage:
    ...
    def create_thread_with_id(
        ...
        """Create a thread record with a specific thread_id
        
        用于需要指定 thread_id 的场景（如复用 Dify 的 conversation_id）
        """
        with self._lock:
            # 检查 thread_id 是否已存在
            if thread_id in self.threads:
                # 如果已存在，更新 last_accessed_at 并返回
    ...
    def get_file_path(self, file_id: str) -> Optional[str]:
        """Get file path in a thread-safe way"""
        with self._lock:
            if file_id in self.files:
                return self.files[file_id].get("filepath")
            return None

    def update_thread_metadata(self, thread_id: str, metadata_updates: Dict[str, Any]) -> bool:
        """Update thread metadata in a thread-safe way
        
        Args:
            thread_id: Thread ID
            metadata_updates: Dictionary of metadata updates (will be merged with existing metadata)
        
        Returns:
            True if thread exists and was updated, False otherwise
        """
        with self._lock:
            if thread_id not in self.threads:
                return False
            ...
            # 合并元数据
            existing_metadata = self.threads[thread_id].get("metadata", {})
            existing_metadata.update(metadata_updates)
            self.threads[thread_id]["metadata"] = existing_metadata
            
            return True

    def append_thread_metadata_list(self, thread_id: str, metadata_key: str, item: Dict[str, Any]) -> bool:
        """Append an item to a list in thread metadata in a thread-safe way
        
        Args:
            thread_id: Thread ID
            metadata_key: Key in metadata dictionary (e.g., "excel_files")
        ...
        with self._lock:
            ...
            # 确保 metadata 存在
            if "metadata" not in self.threads[thread_id]:
                self.threads[thread_id]["metadata"] = {}
            
            metadata = self.threads[thread_id]["metadata"]
            ...
            # 追加项目
            metadata[metadata_key].append(item)
